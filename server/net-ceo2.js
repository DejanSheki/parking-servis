...
const server = net.createServer((socket) => {
    ...
    socket.on('data', (data) => {
        ...
        const result = file.logFileSensit(lastPacketFile);
        result
        ...
        if (objTabla.checksum === crcData) {
            if (objTabla.adresa === '001') {
                ...
                result
                    ...
                    .catch(err => console.log(err));
            } else if (objTabla.adresa === '089') {
                const file = fileService.getFileServiceInstance();
                const result = file.emptyParkingSpaces2('pio', 'mk');
                result
                    .then(data => {
                        const slMesta = data[0].split('Y');
                        const slMestaDisplej1 = slMesta[1] + slMesta[2] + slMesta[3].replace(/(\r\n|\n|\r)/gm, "");
                    ...
                    .catch(err => console.log(err));
            } else if (objTabla.adresa === '090') {
                const file = fileService.getFileServiceInstance();
                const result = file.emptyParkingSpaces2('mas', 'kam');
                result
                    .then(data => {
                        const slMesta = data[0].split('Y');
                        const slMestaDisplej1 = slMesta[1] + slMesta[2] + slMesta[3].replace(/(\r\n|\n|\r)/gm, "");
            ...
            socket.write('{Nisu poslati svi podaci!}');
            socket.end();
        }
        // kraj novog dela
    });
